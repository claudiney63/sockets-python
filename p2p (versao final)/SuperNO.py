...
class SuperNO:
    ...
    def __init__(self, host, port):
        ...
        self.host = ''
        self.port = 5000
        self.peers_list = [('', 5000)]

        self.cliente = socket.socket(socket.AF_INET, socket.SOCK_STREAM)

        try: # tenta iniciar o socket servidor
            self.servidor = socket.socket(socket.AF_INET, socket.SOCK_STREAM) # ipv4 e TCP
            self.servidor.bind((host, port))
            self.servidor.listen()
        except:
            print('\nNão foi possivel iniciar o socket servidor!\n')
            sleep(5)
            os._exit(0) 
    ...
    def start(self):
        for _ in range(3):
            Thread(target=self.run).start() #inicia as threads
    ...
    def run(self):
        while True:
            ...
            while True:
                #print(self.peers_list) # lista os nos da rede
        ...
        if (destino == "ID"):
            ...
            if (len(self.peers_list) == 2):
                ...
                try:
                    ...
                    return print('\nNão foi possivel iniciar a conexão!\n')
